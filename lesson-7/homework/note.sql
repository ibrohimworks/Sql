...
SELECT P.Category, SUM(SaleAmount) AS TotalSales, AVG(SaleAmount) AS AverageSales
FROM Sales S
JOIN Products P ON S.ProductID = P.ProductID
GROUP BY P.Category;


SELECT COUNT(*) AS HREmployees FROM Employees WHERE Salary = 1;


SELECT Salary, MAX(Salary) AS MaxSalary, MIN(Salary) AS MinSalary
FROM Employees
GROUP BY Salary;


SELECT Salary, AVG(Salary) AS AvgSalary
FROM Employees
GROUP BY Salary;


SELECT Salary, AVG(Salary) AS AvgSalary, COUNT(*) AS EmployeeCount
FROM Employees
GROUP BY Salary;
...
SELECT Salary, SUM(Salary) AS TotalSalary, AVG(Salary) AS AvgSalary
FROM Employees
GROUP BY Salary
...
SELECT CustomerID,
       SUM(CustomerID) AS TotalFreightOver50,
       MIN(CustomerID) AS LeastPurchase
FROM Orders
WHERE CustomerID > 50
GROUP BY CustomerID;
