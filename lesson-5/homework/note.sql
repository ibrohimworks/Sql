...
-- 1. ProductName ustunini Name deb alias (taxallus) bilan o‘zgartirish
SELECT ProductName AS Name
FROM Products;

-- 2. Customers jadvalini Client deb alias bilan nomlash
SELECT *
FROM Customers AS Client;

-- 3. Products va Products_Discounted jadvallaridan ProductName ustunini birlashtirish (takrorlanuvchilarsiz)
SELECT ProductName FROM Products
UNION
SELECT ProductName FROM Products_Discounted;

-- 4. Products va Products_Discounted jadvallarining kesishuvini topish (faqat ikkala jadvalda ham mavjud bo‘lgan ProductName)
SELECT ProductName FROM Products
INTERSECT
SELECT ProductName FROM Products_Discounted;

-- 5. Takrorlanmaydigan mijoz ismlari va ularning mamlakatlarini chiqarish
SELECT DISTINCT FirstName, Country
FROM Customers;

-- 6. Narxga qarab shartli ustun chiqarish: agar Price > 1000 bo‘lsa 'High', aks holda 'Low'
...
-- 7. StockQuantity > 100 bo‘lsa 'Yes', aks holda 'No' bo‘lgan IIF yordamida ustun yaratish
...
-- 2 Medium Level Tasks

-- 1. Products ва Products_Discounted жадвалларидаги ProductName устунларини UNION орқали бирлаштириш (такрорлар олиб ташланади)
SELECT ProductName FROM Products
UNION
SELECT ProductName FROM Products_Discounted;

-- 2. Products жадвалида бор, лекин Products_Discounted да йўқ ProductName ларни кўрсатиш (EXCEPT)
SELECT ProductName FROM Products
EXCEPT
SELECT ProductName FROM Products_Discounted;

-- 3. IIF орқали шартли устун: агар Price > 1000 бўлса 'Expensive', акс ҳолда 'Affordable'
SELECT ProductName, Price,
       IIF(Price > 1000, 'Expensive', 'Affordable') AS PriceCategory
FROM Products;

-- 4. Employees жадвалида ёши 25 дан кичик ёки ойлик маоши 60000 дан ортиқ бўлган ходимларни топиш
SELECT *
FROM Employees
WHERE Age < 25 OR Salary > 60000;

-- 5. Ходим HR бўлимда ишласа ёки EmployeeID = 5 бўлса, унинг ойлигини 10% га ошириш
UPDATE Employees
...
-- 1. CASE ёрдамида SaleAmount бўйича тоифалаш: 'Top Tier' (>500), 'Mid Tier' (200–500), 'Low Tier' (қолганлар)
...
-- 2. Orders жадвалида бор бўлган, лекин Sales жадвалида йўқ бўлган мижозларнинг ID ларини топиш
SELECT CustomerID FROM Orders
EXCEPT
SELECT CustomerID FROM Sales;

-- 3. Quantity асосида чегирма фоизини ҳисоблаш (Orders жадвали): 
-- 1 та буюртма — 3%
-- 2–3 та буюртма — 5%
-- 4 ва ундан кўп — 7%
